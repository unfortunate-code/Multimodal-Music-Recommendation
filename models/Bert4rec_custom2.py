...
class BERTRec_sequential(torch.nn.Module):
    ...
    def build_graph(self):
        ...
        self.out = nn.Linear(self.bert.hidden, self.item_num + 1)  # padding 0
...
class BERT(nn.Module):
    def __init__(self, maxlen, num_items, n_layers, heads, hidden, dropout=0.2):
        ...
        self.embedding = BERTEmbedding(vocab_size=num_items + 2, embed_size=1792, final_embed_size=hidden, maxlen=maxlen, dropout=dropout)
...
class BERTEmbedding(nn.Module):
    def __init__(self, vocab_size, embed_size, final_embed_size, maxlen, dropout=0.1):
        super().__init__()
        ###################
        weight = concat_embedding
        self.token = nn.Embedding(num_embeddings=70231, embedding_dim=embed_size, padding_idx=0).from_pretrained(embeddings=weight, freeze=False)
        self.linear1 = torch.nn.Linear(in_features=embed_size, out_features=embed_size // 4)
        self.linear2 = torch.nn.Linear(in_features=embed_size // 4, out_features=final_embed_size)
        ###################
