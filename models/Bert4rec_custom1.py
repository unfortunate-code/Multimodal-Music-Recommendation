...
class BERTRec_sequential(torch.nn.Module):
    ...
    def build_graph(self):
        ...
        self.bert1 = BERT(self.maxlen, self.item_num, self.n_layers, self.heads, self.hidden, 0.3, lyrics_embedding, 512)
        self.norm1 = torch.nn.LayerNorm(self.hidden)
        self.bert2 = BERT(self.maxlen, self.item_num, self.n_layers, self.heads, self.hidden, 0.3, artist_embedding, 320)
        self.norm2 = torch.nn.LayerNorm(self.hidden)
        self.bert3 = BERT(self.maxlen, self.item_num, self.n_layers, self.heads, self.hidden, 0.3, image_embedding, 384)
        self.norm3 = torch.nn.LayerNorm(self.hidden)
        self.bert4 = BERT(self.maxlen, self.item_num, self.n_layers, self.heads, self.hidden, 0.3, audio_embedding, 224)
